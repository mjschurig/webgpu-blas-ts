 ...
 * 
 * The vector x serves as both input (b) and output (solution).
 * Only the upper or lower triangular part of A is referenced,
 * and the matrix can be unit or non-unit triangular.
 * 
 * This is a WebGPU implementation of the reference BLAS STRSV function.
 */

/// <reference types="@webgpu/types" />

import { getWebGPUContext } from '../../webgpu/context';
import strsvShader from '../../shaders/strsv.wgsl';

/**
 * STRSV parameters - reusing types from TRMV
 */
export interface STRSVParams {
  /** Specifies whether A is upper ('U') or lower ('L') triangular */
  uplo: 'U' | 'L';
  /** Specifies the operation: 'N' = A * x = b, 'T' = A^T * x = b */
  trans: 'N' | 'T';
  ...
  lda: number;
  /** Vector x (n elements) - input b, output solution x - modified in-place */
  x: Float32Array;
...
/**
 * Triangular solve using WebGPU (single precision)
 * Solves: A * x = b or A^T * x = b, where x is both input and output
 */
export async function strsv(params: STRSVParams): Promise<void> {
  const { uplo, trans, diag, n, A, lda, x, incx = 1 } = params;
  
  // Validate inputs
  if (n <= 0) return;
  if (incx === 0) return;
  if (lda < n) throw new Error('STRSV: lda must be >= n');
  if (A.length < lda * n) throw new Error('STRSV: Matrix A is too small');
  
  // Validate vector size
  if (x.length < (n - 1) * Math.abs(incx) + 1) {
    throw new Error('STRSV: Vector x is too small');
  }
  ...
  // Dispatch single workgroup (sequential solve requires serialization)
