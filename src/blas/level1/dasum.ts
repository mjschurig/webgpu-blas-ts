 ...
 * 
 * Computes the sum of absolute values of elements in a vector.
 * This is a WebGPU implementation of the reference BLAS DASUM function.
 */
...
/**
 * Compute the sum of absolute values of a vector using WebGPU
 */
export async function dasum(params: DASUMParams): Promise<number> {
  const { n, dx, incx = 1 } = params;
  
  // Validate inputs
  if (n <= 0) return 0.0;
  if (incx <= 0) return 0.0;
  if (dx.length === 0) return 0.0;
  ...
  // Create buffers
  const inputBuffer = context.createBuffer(dx);
  const workspaceSize = blocks * 8; // One f64 per block
  const workspaceBuffer = context.createOutputBuffer(workspaceSize);
  const resultBuffer = context.createOutputBuffer(8); // Single f64 result
  ...
  const resultView = new Float64Array(resultData);
  const totalSum = resultView[0] || 0.0;
