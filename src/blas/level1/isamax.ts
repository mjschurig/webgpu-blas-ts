 ...
 * This is a WebGPU implementation of the reference BLAS ISAMAX function.
 */

/// <reference types="@webgpu/types" />

import { getWebGPUContext } from '../../webgpu/context';
import isamaxStage1Shader from '../../shaders/isamax_stage1.wgsl';
import isamaxStage2Shader from '../../shaders/isamax_stage2.wgsl';

/**
 * ISAMAX parameters
 */
export interface ISAMAXParams {
...
/**
 * Find the index of the maximum absolute value in a vector using WebGPU (single precision)
 * Returns 1-based index (FORTRAN convention), 0 if n <= 0
 */
export async function isamax(params: ISAMAXParams): Promise<number> {
  ...
  // Stage 1: Parallel reduction within workgroups to find local maximums
  ...
  if (blocks > 1) {
    ...
    const resultView = new Uint32Array(resultData);
    maxIndex = resultView[0] || 0; // First u32 is the index
  }
