/**
 * SNRM2 - Compute Euclidean norm of a vector (single precision)
 * 
 * Computes the Euclidean (L2) norm of vector x: sqrt(sum(x[i]^2)).
 * This is a WebGPU implementation of the reference BLAS SNRM2 function.
 */

/// <reference types="@webgpu/types" />

import { getWebGPUContext } from '../../webgpu/context';
import snrm2Stage1Shader from '../../shaders/snrm2_stage1.wgsl';
import snrm2Stage2Shader from '../../shaders/snrm2_stage2.wgsl';

/**
 * SNRM2 parameters
 */
export interface SNRM2Params {
...
/**
 * Compute the Euclidean norm of a vector using WebGPU (single precision)
 */
export async function snrm2(params: SNRM2Params): Promise<number> {
  const { n, sx, incx = 1 } = params;
  
  // Validate inputs
  if (n <= 0) return 0.0;
  if (incx <= 0) return 0.0;
  if (sx.length === 0) return 0.0;
  ...
  // Stage 1: Parallel reduction within workgroups (compute sums of squares)
  ...
  // Stage 2: Final reduction with square root (only if we have multiple blocks)
  if (blocks > 1) {
    ...
    // If only one block, we need to apply square root to the workspace result
    // For simplicity, we'll read the workspace and compute square root on CPU
  ...
  if (blocks > 1) {
    ...
    norm = resultView[0] || 0.0;
  } else {
    // Single block case - need to apply square root
    const resultData = await context.readBuffer(workspaceBuffer, 4);
    const resultView = new Float32Array(resultData);
    const sumOfSquares = resultView[0] || 0.0;
