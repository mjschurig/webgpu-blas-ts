...
test.describe('WebGPU BLAS E2E Tests', () => {
  ...
  test.describe('SASUM - Single Precision Absolute Sum', () => {
    ...
    test('should handle stride correctly', async ({ page }) => {
      ...
      expect(result).toBeCloseTo(6.0, 4); // |1| + |2| + |3|
    });
    ...
    test('should return 0 for edge cases', async ({ page }) => {
      const results = await page.evaluate(async () => {
        return {
          emptyVector: await window.WebGPUBLAS.sasum({ n: 0, sx: new Float32Array([]) }),
          negativeN: await window.WebGPUBLAS.sasum({ n: -1, sx: new Float32Array([1, 2, 3]) }),
          zeroIncx: await window.WebGPUBLAS.sasum({ n: 3, sx: new Float32Array([1, 2, 3]), incx: 0 }),
        };
      });
  ...
  test.describe('SAXPY - Single Precision A*X Plus Y', () => {
    ...
    test('should handle alpha = 0 (no-op)', async ({ page }) => {
      const result = await page.evaluate(async () => {
        const sx = new Float32Array([1, 2, 3, 4]);
        const sy = new Float32Array([5, 6, 7, 8]);
        const original = Array.from(sy);
        
        await window.WebGPUBLAS.saxpy({ n: 4, alpha: 0.0, sx, sy });
        return { original, result: Array.from(sy) };
      });
    ...
    test('should handle different increments', async ({ page }) => {
      const result = await page.evaluate(async () => {
        const sx = new Float32Array([1, -999, 2, -999, 3, -999, 4]);
        const sy = new Float32Array([10, -999, 20, -999, 30, -999, 40]);
        const alpha = 2.0;
        
        await window.WebGPUBLAS.saxpy({ n: 4, alpha, sx, sy, incx: 2, incy: 2 });
        return Array.from(sy);
      });
      
      // Expected: y = 2*x + y at indices 0, 2, 4, 6
      expect(result[0]).toBeCloseTo(12, 4); // 2*1 + 10
      expect(result[1]).toBeCloseTo(-999, 4); // unchanged
      expect(result[2]).toBeCloseTo(24, 4); // 2*2 + 20
      expect(result[3]).toBeCloseTo(-999, 4); // unchanged
      expect(result[4]).toBeCloseTo(36, 4); // 2*3 + 30
      expect(result[5]).toBeCloseTo(-999, 4); // unchanged
      expect(result[6]).toBeCloseTo(48, 4); // 2*4 + 40
    });
  });
  ...
  test.describe('Performance Tests', () => {
    test('should handle medium-sized vectors efficiently', async ({ page }, testInfo) => {
      const { duration, passed } = await page.evaluate(async () => {
        ...
        const sx = new Float32Array(n);
        for (let i = 0; i < n; i++) {
          sx[i] = Math.random() * 2 - 1; // Random between -1 and 1
        }
