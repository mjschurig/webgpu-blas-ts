/**
 * ISAMAX Stage 1 Shader - Find local maximum absolute values and indices within workgroups
 * 
 * Computes partial maximum absolute values and their indices within each workgroup.
 * Each workgroup processes WORKGROUP_SIZE * WIN elements and outputs one max value/index pair.
 * This is the WebGPU implementation of the first stage of BLAS ISAMAX operation.
 */
...
fn main(
  ...
  var local_max = IndexValue(0u, 0.0f); // 0 index means invalid
  ...
  for (var i: u32 = 0u; i < WIN; i = i + 1u) {
    ...
    if (idx < n) {
      ...
      if (x_idx < arrayLength(&x)) {
        ...
        if (local_max.index == 0u || abs_val > local_max.value || 
           (abs_val == local_max.value && one_based_idx < local_max.index)) {
          local_max.index = one_based_idx;
