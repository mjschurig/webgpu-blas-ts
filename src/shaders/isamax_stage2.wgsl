/**
 * ISAMAX Stage 2 Shader - Final reduction to find global maximum
 * 
 * Combines the partial maximum values and indices from all workgroups into a single final result.
 * This stage runs only when multiple workgroups were used in stage 1.
 * This is the WebGPU implementation of the second stage of BLAS ISAMAX operation.
 */
...
fn main(
  ...
  while (idx < blocks) {
    ...
    if (other.index != 0u && (local_max.index == 0u || 
        other.value > local_max.value ||
        (other.value == local_max.value && other.index < local_max.index))) {
      local_max = other;
    }
    idx += WORKGROUP_SIZE;
  }
