/**
 * SDOT Stage 1 Shader - Parallel reduction within workgroups for dot product
 * 
 * Computes partial dot products within each workgroup.
 * Each workgroup processes WORKGROUP_SIZE * WIN elements and outputs one sum.
 * This is the WebGPU implementation of the first stage of BLAS SDOT operation.
 */
...
fn main(
  ...
  for (var i: u32 = 0u; i < WIN; i = i + 1u) {
    ...
    if (idx < n) {
      // Calculate actual array indices with strides
      let x_idx = u32(max(0, i32(idx) * incx));
      let y_idx = u32(max(0, i32(idx) * incy));
      
      if (x_idx < arrayLength(&x) && y_idx < arrayLength(&y)) {
        // Compute dot product: sum += x[i] * y[i]
        sum += x[x_idx] * y[y_idx];
      }
    }
  }
